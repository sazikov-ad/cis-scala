...
object task2 extends App {
  ...
  def traverse[A](list: List[Option[A]]): Option[List[A]] = list
    .foldLeft(Some(List()): Option[List[A]]) {
      case (_, None) => None
      case (None, _) => None
      case (Some(l), Some(v)) => Some(v :: l)
    }
    .map(_.reverse)
  ...
  def splitEither[A, B](list: List[Either[A, B]]): (List[A], List[B]) = list
    .foldLeft((List[A](), List[B]())) {
      case ((l, r), Left(v)) => (v :: l, r)
      case ((l, r), Right(v)) => (l, v :: r)
    }
  ...
  def validate[A, B](list: List[Either[A, B]]): Either[List[A], List[B]] = list
    .foldLeft(Right(List()): Either[List[A], List[B]]) {
      case (r@Left(a), Right(_)) => r
      case (Left(a), Left(v)) => Left(v :: a)
      case (Right(_), Left(v)) => Left(v :: Nil)
      case (Right(a), Right(v)) => Right(v :: a)
    }
